...
def inicializar_csv():
    if not os.path.exists(CSV_FILE):
        with open(CSV_FILE, mode="w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                "timestamp", "planta", "fase",
                "luminosidade", "temp_ar", "umid_ar",
                "temp_solo", "umid_solo",
                "aquecedor_estado", "aquecedor_motivo",
                "ventoinha_estado", "ventoinha_motivo",
                "luminaria_estado", "luminaria_motivo",
                "bomba_estado", "bomba_motivo"
            ])
...
def teste_logger():
    ...
    while True:
        try:
            ...
            # 🔹 Atuadores → Firestore
            docs = firestore_db.collection("Dispositivos").document(ESTUFA_ID).collection("Dados").get()
            atuadores = {
                "Aquecedor": (None, None),
            ...
            with open(CSV_FILE, mode="a", newline="") as f:
                ...
                writer.writerow([
                    ...
                    atuadores["Aquecedor"][0], atuadores["Aquecedor"][1],
                    atuadores["Ventilacao"][0], atuadores["Ventilacao"][1],
                    atuadores["Luminaria"][0], atuadores["Luminaria"][1],
                    atuadores["Bomba"][0], atuadores["Bomba"][1],
                ])
